       ...
       */
       ...
       //----------------------------------------------------------------------------------------------------
        ...
        if ($params['is_recur'] == true) {
            ...
            //$params['invoiceID'] = substr($params['invoiceID'], 0, 8) . '(r)';
       		// We wont actually store the Invoice ID yet, mainly because we can't seem to modify it and the default is too long and doesn't enable us to add the required (r) to IDs. So instead, we'll take the number generated by Civi and pass it to eWay as a reference for future use.
       		// CLIENT SPECIFIC
       		$uniqueTrnxNum = substr($params['invoiceID'], 0, 8);     
       		$uniqueTrnxNum = $uniqueTrnxNum . '(r)' ;
       		
       		// Create request object, this is fairly similar to original eWay processor with a few changes to reflect fields required.
       		$RebillPayment->ewayCustomerID($ewayCustomerID);  //    8 Chars - ewayCustomerID                 - Required 
       		$RebillPayment->RebillInitAmt($amountInCents);  //   12 Chars - ewayTotalAmount  (in cents)    - Required
            $RebillPayment->RebillInitDate($currDate);  //    2 Chars - ewayCardExpiryYear             - Required 
			...
			$RebillPayment->RebillInvRef($params['contributionID'].'(r)');  //   50 Chars - ewayCustomerInvoiceRef
		   	...
		   	$RebillPayment->RebillCCExpYear($expireYear);  //    2 Chars - ewayCardExpiryYear             - Required 
			$RebillPayment->RebillStartDate($currDate);  //    2 Chars - ewayCardExpiryYear             - Required 
			$RebillPayment->RebillInterval($params['frequency_interval']);  //    2 Chars - ewayCardExpiryYear             - Required 
			// CLIENT SPECIFIC, We default to a year of monthly payments
			if ($params['installments'] =='') $params['installments'] = 12 ;

			$intervalType = 0 ;

			//These are determined by eWay
			if ($params['frequency_unit'] == 'day') $intervalType = 1 ;
			if ($params['frequency_unit'] == 'week') $intervalType = 2 ;
			if ($params['frequency_unit'] == 'month') $intervalType = 3 ;
			if ($params['frequency_unit'] == 'year') $intervalType = 4 ;

        	$RebillPayment->RebillIntervalType($intervalType);  //    2 Chars - ewayCardExpiryYear             - Required 

			// Generate the number of payments to be made
 			$endNo = floor($params['installments'] / $params['frequency_interval']) ;
 			// Generate the end date of payments
  			$endDate = date('d/m/Y', strtotime('+' . $endNo . $params['frequency_unit'])) ;

        	$RebillPayment->RebillEndDate($endDate);  //    2 Chars - ewayCardExpiryYear             - Required 
